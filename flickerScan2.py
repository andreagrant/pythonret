...
def flickerScan(scanDict, screenSize=[1024,768]):
    ...
    colorA=numpy.zeros((3))
    colorB=numpy.zeros((3))
    colorBG=numpy.zeros((3))


    foo=scanDict['colorA']
    if type(foo) is str:
        #this is the option that comes from the dynamic mode/dialog box
    ...
    if screenCount==1:
        ...
        winOp = visual.Window([700,500],monitor='testMonitor',units='deg',screen=scanDict['operatorScreen'],
                              color=[0.0,0.0,0.0],colorSpace='rgb')
        msgScanLength=visual.TextStim(winOp,pos=[0,0.5],units='norm',height=0.1,text='Scan length (s): %.1f' %scanLength)
        msgScanTr=visual.TextStim(winOp,pos=[0,0],units='norm',height=0.1,text='No. of Volumes (at Tr=%.2f): %.1f' %(scanDict['Tr'],scanLength/scanDict['Tr']) )
        msgScanLength.draw()
    ...
    print('flickerScan2 line 126',colorBGp)
    winSub = visual.Window(screenSize,monitor=scanDict['monCalFile'],units='deg',screen=mySubScreen,
                       color=colorBGp,colorSpace='rgb',fullscr=False,allowGUI=False)
    ...
    theta2=numpy.arctan2(x,y)
    theta2[halfSize+1,halfSize+1]=0
    #number of wedges (pairs!!)--eventually to be a var passed in
    if 'numWedgePairs' in scanDict:
        #have the size
        nWedges=scanDict['numWedgePairs']
    else:
        nWedges=8.0
    #number of ring pairs
    if 'ringWidth' in scanDict:
  ...
  #  plt.imshow(ringFunction); plt.colorbar()

    wedgeMask = 0.5 - (numpy.mod(theta2,wedgeWidth)>(wedgeWidth/2.0)) #does this work


    mon=monitors.Monitor(scanDict['monCalFile'])
    screenSubtentHalf=(180.0/math.pi)*numpy.arctan2(0.5*mon.getWidth(),mon.getDistance())
    ringWidthNorm=ringWidth/screenSubtentHalf
    rmA=numpy.mod(ringFunction,ringWidthNorm)>(ringWidthNorm/2.0)
    ringMask = 1 - 2.0*(rmA)
 #   plt.imshow(rmA); plt.colorbar()

    checkerBoardLogic=wedgeMask*ringMask + 0.5
#    plt.imshow(checkerBoardLogic); plt.colorbar()
...
#    checkerBoardAB[r>halfSize]=colorBGp[2]
    #nope--need to mask out anyhting outside OR!
    ORpix=OR*(screenSize[0]/2.0)/screenSubtentHalf
    checkerBoardAR[r>ORpix]=colorBGp[0]
    checkerBoardAG[r>ORpix]=colorBGp[1]
    checkerBoardAB[r>ORpix]=colorBGp[2]
...
#    checkerBoardBB[r>halfSize]=colorBGp[2]
    checkerBoardBR[r>ORpix]=colorBGp[0]
    checkerBoardBG[r>ORpix]=colorBGp[1]
    checkerBoardBB[r>ORpix]=colorBGp[2]
...
#    numpy.savetxt('chAr.txt',checkerBoardA[:,:,0],fmt='%f')
    ...
    stimA=visual.GratingStim(winSub,tex=checkerBoardA,size=imageSize,units='pix')
    stimB=visual.GratingStim(winSub,tex=checkerBoardB,size=imageSize,units='pix')
    ...
    if screenCount==2:
        msg = visual.TextStim(winOp,units='pix',text = 't = %.3f' %timeNow, pos=(0.0,220.0),height=30)
        msg.draw()
        winOp.flip()
    loopCounter=0
    restLoopCounter=0
    ...
    if timeBase==0:
        ttp=TrCounter+1
        if screenCount==2:
            msgTr=visual.TextStim(winOp,units='pix',pos=(0.0,220.0),text='Tr = %i' %ttp,height=30)
            msgTr.draw()
            winOp.flip()
...
#                    print 'aha'
        ...
        while TrCounter<numTr:
            ...
            for key in event.getKeys():
                if key in ['q','escape']:
                    ...
                    subjectResponse[numCoins]=1
                    subRespArray[respCounter,2]=1
                elif key in triggerKeys and TrDone==0:
    ...
    outFile='flickerResponse%04d%02d%02d_%02d%02d.txt'%(nowTime.year,nowTime.month,nowTime.day,nowTime.hour,nowTime.minute)
