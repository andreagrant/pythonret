...
################### 3 masks ###################################
#############################################################################
def driftChecker3mask(scanDict,screenSize=[1024,768],offTimeBehavior=1,maskType=1):
    #do wedge
    #length of scan in s
...
#        myAnn2=myAnn2a*2-1 + myAnn2b*2-1
        ...
        myAnn3=(numpy.where(myfilter<Rbeta,1,0)+numpy.where(myfilter>OR,1,0))*2-1
        maskC=visual.GratingStim(winSub,tex=None,mask=myAnn3,units='pix',size=screenSize[0],color=gray,colorSpace='rgb')
...
#                nowPhase=startPhases + deltaPhase*(phaseSignVec)
    ...
    outFile='drift3Response%04d%02d%02d_%02d%02d.txt'%(nowTime.year,nowTime.month,nowTime.day,nowTime.hour,nowTime.minute)
