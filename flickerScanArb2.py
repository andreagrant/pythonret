...
def flickerScanArb(scanDict, screenSize=[1024,768]):
    ...
    debugVar=numpy.zeros((int(scanLength*60),2))
    if screenSize[0]<257:
        imageSize=256
...
#    checkerBoardBB[r>halfSize]=colorBGp[2]
    ...
    #wait for subject
    if screenCount==2:
        msgTr=visual.TextStim(winOp,units='pix',pos=(0.0,200.0),text='Tr = 0')
        msgScanLength.draw()
        msgScanTr.draw()
        msg = visual.TextStim(winOp,units='pix',pos=(0.0,200.0),text = 'waiting for subject to press a button')
        msg.draw()
        winOp.flip()
    thisKey=None
    ...
    if screenCount==2:
        msgScanLength.draw()
        msgScanTr.draw()
        msg.setText('waiting for MAGNET TRIGGER')
        msg.draw()
        msg.setPos([0,200])
        winOp.flip()
